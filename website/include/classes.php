   ...
   public function auditlog($context, $message, $priority = 0, $userid = NULL){
    ...
    public function register($username, $password, $email, $registrationcode, &$errors) {
        ...
        if (sizeof($errors) == 0) {
			...
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $url);
			curl_setopt($ch, CURLOPT_HTTPHEADER, array('x-api-key  : vlKjhk38cF7oPyvchcZAi95vIZALWdBe25ypO6xI'));
			curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
			curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			$response  = curl_exec($ch);
...
/*
    // Retrieves an existing session from the database for the specified user
        public function getSessionUser(&$errors, $suppressLog=FALSE) {
          ...
          $url = "https://2wz4hwder1.execute-api.us-east-1.amazonaws.com/default/getSession?usersessionid=" . $sessionid;
    			$ch = curl_init();
    			curl_setopt($ch, CURLOPT_URL, $url);
    			curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
    			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    			curl_setopt($ch, CURLOPT_HTTPHEADER('x-api-key : aVGDnXziqqaNV0Uz6yg1S6nKlVJWA40g1gYtPt1l');
    			$response  = curl_exec($ch);
    			$httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    			...
    			if ($response === FALSE) {
    				...
    				if($httpCode == 400) {
    					...
    					$this->debug("web service response => " . $response);
    					$tempUser = json_decode($response);
    					//$this->auditlog("getSessionUser", "web service response => ");
    					$this->debug("success");
    				}
    			}
...
/*
    ...
    public function isAdmin(&$errors, $userid) {
        ...
        $url = "https://b1cwviqgyh.execute-api.us-east-1.amazonaws.com/default/isAdmin";

    		$ch = curl_init();
    		curl_setopt($ch, CURLOPT_URL, $url);
    		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
    		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    		curl_setopt($ch, CURLOPT_HTTPHEADER, array('x-api-key : Zpc3mLRMqc7BKPTS3YteY1Y4A403G49rXs2oC0e4'));
    		$response  = curl_exec($ch);
    		$httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    ...
    // Logs out the current user based on session ID
        public function logout() {
            ...
            if (!empty($sessionid)) {
                 ...
                 $url = 'https://lrziayywf8.execute-api.us-east-1.amazonaws.com/default/logoutUser?sessionid';

                 $data_json = json_encode($data);
                 $ch = curl_init();
    			curl_setopt($ch, CURLOPT_URL, $url);
    			 curl_setopt($ch, CURLOPT_HTTPHEADER, array('x-api-key : a8GgPLsFOJ86ukw1aIbe79vNcZ4tQKX3agyxMEgv'));


                curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'DELETE');
    			curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'DELETE');
    			curl_setopt($ch, CURLOPT_DELETEFIELDS, $data_json);
    			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    			$response  = curl_exec($ch);
    			$httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
...
/*
    public function getThings(&$errors) {
		...
		$user = $this->getSessionUser($errors);
        $registrationcode = 'it5236';
		
		$url = "https://yfe5t8pkw8.execute-api.us-east-1.amazonaws.com/default/getThings?registrationcode=".$registrationcode;
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
		curl_setopt($ch, CURLOPT_HTTPHEADER, array('x-api-key : gN9wWhRyxB90BnQdnZlsx9WxYMUjm2Fklpzzf16a'));
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		$response  = curl_exec($ch);
		$httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
		$this->debug($response);
...
/*
    public function getComments($thingid, &$errors) {
    		...
    		$ch = curl_init();
    		curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('x-api-key : rcSNeOdBEF5RUTqhWEoqO3cZMY5zaFXE8QAdTIc1'));
    		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
    		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    		$response  = curl_exec($ch);
    		$httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    		...
    		if ($response === FALSE) {
    			...
    			if($httpCode == 400) {
    				...
    				// JSON was double-encoded, so it needs to be double decoded
    				$errorsList = json_decode(json_decode("string",$response))->errors;
    				foreach ($errorsList as $err) {
    					$errors[] = $err;
    				}
    				if (sizeof($errors) == 0) {
    				...
    				$comment_object = json_decode($response, true);
